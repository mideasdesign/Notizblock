...
function getArchiveNotesTemplate(indexArchiveNote) {
  return `<div class="grid grid-rows-3 sm:grid-cols-3 w-full"> <h3>+ ${notesTitlesArchive[indexArchiveNote]}</h3> <p> ${notesArchive[indexArchiveNote]}</p> <button class="py-1 border-2 border-gray-300 rounded-xl px-2 hover:cursor-pointer hover:bg-gray-300" onclick="archiveToTrash(${indexArchiveNote})"> delete </button></div>`;
}