...
function getArchiveNotesTemplate(indexArchiveNote, indexArchiveNoteTitle) {
  return `<details class="p-6 border my-4 border-transparent open:border-black/10 open:bg-gray-100 open:rounded-xl">
        <summary class="flex justify-between hover:cursor-pointer text-sm leading-6 font-semibold text-gray-900 select-none">
            <h3 id="archive-notes-title" class="text-xl text-gray-600 flex">${notesArchiveTitles[indexArchiveNoteTitle]}</h3>
            </summary>
        <div id="notes-list" class="mt-3 text-sm leading-6 text-gray-600">
            <p>${notesArchive[indexArchiveNote]} </p>
            <button class="py-1 border-2 border-gray-300 rounded-xl px-2 hover:cursor-pointer hover:bg-gray-300" onclick="emtyTrash(${indexArchiveNote})"> delete </button>
        </div>
        </details>  
  `;
}
